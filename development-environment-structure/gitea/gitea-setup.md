...
この際、.gitignoreに利用するプログラミング言語やIDEを設定することで、不要なファイルを自動で除外することができます。不要なファイルとは、ソースコードのビルド成果物や、開発者個人の設定ファイルなどです。

[GitHubによる.gitignoreの解説](https://docs.github.com/ja/get-started/getting-started-with-git/ignoring-files)

.gitignoreはテンプレートに用意されていないものもあります。その場合は、自分で作成する必要があります。

[.gitignoreを作成するためのサイト](https://www.toptal.com/developers/gitignore)もありますので、そちらを利用すると便利です。
